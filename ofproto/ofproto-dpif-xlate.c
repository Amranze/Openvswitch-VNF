...
static void compose_vnf_action(struct xlate_ctx *ctx, struct ofpact_vnf_action *action,
							   struct ofpbuf *odp_actions); //kahina
                        ...
                        const struct xlate_bond_recirc *xr, bool check_stp)
{
    ...
    if (xport->peer) {
        ...
        VLOG_WARN("kahina compose output action...  if (xport->peer)t %d",0);
        return;
    }
    ...
    if (xport->is_tunnel) {
        ...
        if (odp_port == ODPP_NONE) {
            ...
            VLOG_WARN("kahina compose output action odp_port == ODPP_NONE %d",0);
        ...
        if (ipv6_addr_equals(&dst, &ctx->orig_tunnel_ipv6_dst)) {
            ...
            VLOG_WARN("kahina compose output action goto out %d",0);
            goto out; /* restore flow_nw_tos */
        }
        if (ctx->xin->resubmit_stats) {
            netdev_vport_inc_tx(xport->netdev, ctx->xin->resubmit_stats);
    ...
    if (out_port != ODPP_NONE) {
        ...
        if (xr) {
            ...
            /* Hash action. */
            VLOG_WARN("kahina compose output action . OVS_ACTION_ATTR_HASH %d",0);
        	...
        	VLOG_WARN("kahina compose output action . OVS_ACTION_ATTR_OUTPUT %d",0);
                          ...
                          uint16_t controller_id)
{
    ...
    VLOG_WARN("kahina lib ofprotot dpif execute_controller_action %d",0);
                    ...
                    ofp_port_t port, uint16_t max_len, bool may_packet_in)
{
    ...
    switch (port) {
    case OFPP_IN_PORT:
    	VLOG_WARN("kahina OFPP_IN_PORT %d",0);
        compose_output_action(ctx, ctx->xin->flow.in_port.ofp_port, NULL);
        break;
    case OFPP_TABLE:
    	VLOG_WARN("kahina OFPP_TABLE %d",0);
        xlate_table_action(ctx, ctx->xin->flow.in_port.ofp_port,
                           0, may_packet_in, true);
        break;
    case OFPP_NORMAL:
    	VLOG_WARN("kahina OFPP_normal %d",0);
        xlate_normal(ctx);
        break;
    case OFPP_FLOOD:
    	VLOG_WARN("kahina OFPP_flood %d",0);
        flood_packets(ctx,  false);
        break;
    case OFPP_ALL:
    	VLOG_WARN("kahina OFPP_all %d",0);
        flood_packets(ctx, true);
        break;
    case OFPP_CONTROLLER:
    	VLOG_WARN("kahina OFPP_controler %d",0);
        ...
        break;
    case OFPP_NONE:
    	VLOG_WARN("kahina OFPP_none %d",0);
        break;
    case OFPP_LOCAL:
    	VLOG_WARN("kahina OFPP_local %d",0);
    default:
        if (port != ctx->xin->flow.in_port.ofp_port) {
        	VLOG_WARN("kahina OFPP_prt default compose_output_action  %d",0);
...
xlate_action_set(struct xlate_ctx *ctx)
{
    ...
    VLOG_WARN("kahina .xlate_action_set........ %d",0);
...
void compose_vnf_action(struct xlate_ctx *ctx, struct ofpact_vnf_action *action,
		struct ofpbuf *odp_actions )
{
	struct ovs_action_vnf vnf_action;
	struct ofproto_dpif *of =ctx->xin->ofproto;
	struct vnftable *tbl;
	int vnfid;

	VLOG_WARN("kahina.compose vnf action ......... %d",0);
    ...
    if(vnfid >  ENTRY_NBR )// tbl->nb_entry )
      { VLOG_WARN("kahina.compose_vnf_action   |bad vnfid  %d",vnfid);
       return;}



    ovs_mutex_lock(&tbl->rwlock);
    if (tbl->vnf_table[vnfid].used==true)
    {
        VLOG_WARN("kahina.compose_vnf_action   |entry exist  %d",0);
        strncpy(vnf_action.extern_library ,tbl->vnf_table[vnfid].extern_library,32);
        VLOG_WARN("kahina.compose_vnf_action   |strncpy ok| %s",vnf_action.extern_library);
		strncpy(vnf_action.fnct_name,tbl->vnf_table[vnfid].fnct_name,32);
		VLOG_WARN("kahina.compose_vnf_action   |strncpy ok|%s",vnf_action.fnct_name);
		vnf_action.fnct_parametre=  tbl->vnf_table[vnfid].internal_parameters;
		VLOG_WARN("kahina.compose_vnf_action   |copy param intern  %d",0);
		vnf_action.external_parameters =  tbl->vnf_table[vnfid].external_parameters;
		VLOG_WARN("kahina.compose_vnf_action   |copy param  extern  %d",0);

		VLOG_WARN("kahina   |  extern_library   %s",vnf_action.extern_library);
  // debut test
  //strncpy(vnf_action.fnct_name,"replace_ip",32);
  //vnf_action.fnct_parametre=NULL;
  // fin  test
  nl_msg_put_unspec(&ctx->xin->odp_actions, OVS_ACTION_ATTR_VNF,&vnf_action, sizeof(vnf_action));
  nl_msg_put_unspec(odp_actions, OVS_ACTION_ATTR_VNF,&vnf_action, sizeof(vnf_action));

  VLOG_WARN("kahina.compose_vnf_action   |nl_msg_put_unspec  %d",0);

  ctx->xout->slow |= SLOW_ACTION;  //
  } else {
	  VLOG_WARN("kahina.compose_vnf_action  |noooo vnf entry  %d",0);
	  /*peut envoyer erreur au controller */
 }
 ovs_mutex_unlock(&tbl->rwlock);
}
                 ...
                 struct xlate_ctx *ctx)
{
    ...
    VLOG_WARN("kahina .do_xlate_action ........%d",0);
    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
        ...
        VLOG_WARN("kahina dans do_xlate_actions avant switch %d ",0);
        if (ctx->exit) {
        ...
        case OFPACT_OUTPUT:
            ...
            VLOG_WARN("kahina dans do_xlate_actions OFPACT_OUTPUT %d ",0);
            break;
		...
		case OFPACT_VNF_ACTION:
			VLOG_WARN("kahina. OFPACT_VNF_ACTION   %d",0);
			compose_vnf_action(ctx, ofpact_get_VNF_ACTION(a), ctx->odp_actions);
			VLOG_WARN("kahina. ctx->xin->packet== null   %d",0);
			break;
        case OFPACT_CT:
            CHECK_MPLS_RECIRCULATION();
...
xlate_actions_for_side_effects(struct xlate_in *xin)
{
    ...
    VLOG_WARN("kahina . xlate action for side effects %d",0);
...
xlate_actions(struct xlate_in *xin, struct xlate_out *xout)
{
    *xout = (struct xlate_out) {
    ...
    VLOG_WARN("kahina .xlate_actions.......... %d",0);
    struct xlate_cfg *xcfg = ovsrcu_get(struct xlate_cfg *, &xcfgp);
    ...
    if (!xin->ofpacts && !ctx.rule) {
        ctx.rule = rule_dpif_lookup_from_table(
            ...
            flow->in_port.ofp_port, true, true, xin->packet);
        if (ctx.xin->resubmit_stats) {
    ...
    if (!xin->recirc && process_special(&ctx, in_port)) {
        ...
        if (tnl_process_ecn(flow)
            ...
            VLOG_WARN("kahina xlate_actions in  do_xlate_actions  %d",0);
            ...
            if (in_port && (!xport_stp_forward_state(in_port) ||
                 ...
                 * not recirculating. */
                if (!exit_recirculates(&ctx)) {
                	VLOG_WARN("kahina xlate action_set.........  %d",0);
                    xlate_action_set(&ctx);
                }
            }
            /* Check if need to recirculate. */
