...
static void rule_criteria_init(struct rule_criteria *, uint8_t table_id,
							   ...
							   int match_nf, cls_version_t version,
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    LIST_FOR_EACH_POP (c, u.list_node, dead_cookies) {
        ...
        rule_criteria_init(&criteria, c->table_id, &match, 0, 0, CLS_MAX_VERSION,
                           c->cookie, OVS_BE64_MAX, OFPP_ANY, OFPG_ANY);
        //Amrane the second 0 is for match_nf
...
rule_criteria_init(struct rule_criteria *criteria, uint8_t table_id,
                   const struct match *match, int priority, int match_nf,
                   cls_version_t version, ovs_be64 cookie,
                   ovs_be64 cookie_mask, ofp_port_t out_port,
                   uint32_t out_group)
{
    criteria->table_id = table_id;
    cls_rule_init(&criteria->cr, match, priority, match_nf); // Amrane i added match_nf
    ...
    if (criteria->cookie_mask == OVS_BE64_MAX) {
        ...
        FOR_EACH_MATCHING_TABLE (table, criteria->table_id, ofproto) {
            ...
            CLS_FOR_EACH_TARGET (rule, cr, &table->cls, &criteria->cr,
                                 criteria->version) {
            	VLOG_WARN("Amrane collect_rules_loose prio %d match_nf %d",
            			rule->cr.priority, rule->cr.match_nf);
                collect_rule(rule, criteria, rules, &n_readonly);
            }
        }
    }
    ...
    if (criteria->cookie_mask == OVS_BE64_MAX) {
        ...
        FOR_EACH_MATCHING_TABLE (table, criteria->table_id, ofproto) {
            ...
            rule = rule_from_cls_rule(classifier_find_rule_exactly(
                                          ...
                                          criteria->version));
            if (rule) {
            	VLOG_WARN("Amrane collect_rules_strict prio %d match_nf %d",
            			rule->cr.priority, rule->cr.match_nf);
                collect_rule(rule, criteria, rules, &n_readonly);
            }
        }
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    rule_criteria_init(&criteria, fsr.table_id, &fsr.match, 0, 0, CLS_MAX_VERSION,
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    rule_criteria_init(&criteria, request.table_id, &request.match, 0, 0,
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    cls_rule_init(&cr, &fm->match, fm->priority, fm->match_nf);
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    rule_criteria_init(&criteria, fm->table_id, &fm->match, 0, fm->match_nf,
    		CLS_MAX_VERSION, fm->cookie, fm->cookie_mask, OFPP_ANY, OFPG_ANY);
    // Amrane  match_nf
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    rule_criteria_init(&criteria, fm->table_id, &fm->match, fm->priority, fm->match_nf,
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    rule_criteria_init(&criteria, fm->table_id, &fm->match, 0, fm->match_nf,
    		CLS_MAX_VERSION, fm->cookie, fm->cookie_mask, fm->out_port,
                       fm->out_group);
    // Amrane match_nf
    rule_criteria_require_rw(&criteria,
                             (fm->flags & OFPUTIL_FF_NO_READONLY) != 0);
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    cls_rule_init_from_minimatch(&target, &m->match, 0, 0); //Amrane 0 for match_nf
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    match_init_catchall(&match);
    rule_criteria_init(&criteria, 0xff, &match, 0, CLS_MAX_VERSION, htonll(0), htonll(0),
                       htonll(0), OFPP_ANY, group->group_id);
    // Amrane the second htonll(0) is for match_nf
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    VLOG_WARN("Amrane  handle_openflow__   %s",str_pack);
...
ofproto_get_vlan_usage(struct ofproto *ofproto, unsigned long int *vlan_bitmap)
{
    ...
    cls_rule_init(&target, &match, 0, 0); //Amrane 0 for match_nf
