...
{
	/*START KAHINA */
    VLOG_WARN("kahina test module alloc %d",0);
    /*END KAHINA */
    struct ofproto_dpif *ofproto = xzalloc(sizeof *ofproto);
    VLOG_WARN("kahina test module xmalloc %d",0);
    return &ofproto->up;
}
...
{
	struct vnftable *table = ofproto_->vnf_table;  //kahina
    struct ofproto_dpif *ofproto = ofproto_dpif_cast(ofproto_);
    ...
    ofproto->vnf_table=table; // kahina
...
destruct(struct ofproto *ofproto_)
{
    ...
    /*START KAHINA*/
	vnftable_destroy(&ofproto->up);
	VLOG_WARN("kahina test module vnftable_destroy %d",0);
    /*END KAHINA*/
...
rule_dpif_lookup_in_table(struct ofproto_dpif *ofproto, cls_version_t version,
                          ...
                          struct flow_wildcards *wc, const struct dp_packet *packet)
{
    //FIXME Check always the packet if NULL because it's NULL for adding internal flow

    struct classifier *cls = &ofproto->up.tables[table_id].cls;
    return rule_dpif_cast(rule_from_cls_rule(classifier_lookup(cls, version,flow, wc,
    														   packet, ofproto->vnf_table)));
}
...
rule_dpif_lookup_from_table(struct ofproto_dpif *ofproto,
							...
							const struct dp_packet *packet)
{
	/* Amrane i added const struct dp_packet *packet*/
    ...
    /*char * str_packet = dp_packet_to_string(packet,  packet->size_);
    VLOG_WARN("Amrane >>>>>>>>>> rule_dpif_lookup_from_table <<<<<<<<<<< "
    		" %s",str_packet);*/
    ...
    {
        *table_id = next_id;
        rule = rule_dpif_lookup_in_table(ofproto, version, next_id, flow, wc, packet);
        if (stats) {
...
{
	/*START KAHINA */
    VLOG_WARN("kahina :::rule construct    %d",0);
    /*END KAHINA */
    struct rule_dpif *rule = rule_dpif_cast(rule_);
    int error;
...
{
	/*START KAHINA */
	VLOG_WARN("kahina appel rule insert %s",0);
    /*END KAHINA */
    struct rule_dpif *rule = rule_dpif_cast(rule_);
              ...
              struct ds *ds)
{
    ...
    VLOG_WARN("kahina . ofproto_trace revalidate_ukey %d",0);
                               ...
                               struct rule **rulep)
{
    ...
    rule = rule_dpif_lookup_in_table(ofproto,
                                     ...
                                     &ofm.fm.match.wc, NULL);
    /* I added NULL for the packet*/
    if (rule) {
        *rulep = &rule->up;
    } else {
        OVS_NOT_REACHED();
