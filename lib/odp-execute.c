...
VLOG_DEFINE_THIS_MODULE(odp_execute_c);

void *vnf_lib;
//char *odp_extern_library= "/home/air/lib_vnffunctions/libvnfUsr.so";
char *odp_extern_library= "/home/air/lib_vnffunctions/LibVnfUsr.so";
                   ...
                   odp_execute_cb dp_execute_action)
{
    ...
    VLOG_WARN("kahina odp_execute odp_execute sample  %d",0);
...
char *remove_begin_space (char *str, int len)
{
    	char *s;
    	int index=0, i=0;
        s = str;
    	while(*s==0x20)  { s++;index++;}
    	//VLOG_WARN("kahina chaine index : %d  ",index );
    	//VLOG_WARN("kahina chaine length : %d  ",len -index );

    	char *fn_name= malloc ((len-index+1)*sizeof(char));

    	for (i=0;i<len-index;i++) {fn_name[i]=*s; s++;}
    	fn_name[len-index]= '\0';

    	VLOG_WARN("kahina chaine transformé : %s  ",fn_name );


    	if (strcmp(fn_name,"replace_adr")==0){VLOG_WARN("kahina transform string : equal ");}
    	else VLOG_WARN("kahina transform string : no equal ");

return fn_name;

}

int  open_library(char *library_Name){
	 ...
	 VLOG_WARN("kahina  library dir %s",library);
	vnf_lib=dlopen(library,RTLD_LAZY);

	if(!vnf_lib)
	{ VLOG_WARN("kahina error in library %d",0);
	  return false;
	}

   VLOG_WARN("kahina . odp action  open_library ok  %d",0);
    return true;
...
int call_function(struct ovs_action_vnf *action, struct dp_packet  *p)
{char * error_;
 int x=0;char *name;
 ...
 /*************/
 name=remove_begin_space (action->fnct_name, 32);


VLOG_WARN("kahina : call_function : function to call:%s: ",name );
 /*************/

 func_call=dlsym(vnf_lib,name);  // replace  "*" by action->fnct_name);

 VLOG_WARN("kahina function_call after call replace ip    %d",0);
  if((error_=dlerror())!=NULL)
  {   VLOG_WARN("kahina erreur dans lib externe ");
      return 1;
  }
 VLOG_WARN("kahina function_callno error     %d",0);

   x=(*func_call)(p, action->fnct_parametre );
   VLOG_WARN("kahina appel fonction externe call   ok code retour -->%d ",x);

return 0;
}


void execute_odp_vnf_action(struct dp_packet *packet  ,struct ovs_action_vnf  *action)
{
	int error;
	char *name_library;
	name_library = remove_begin_space (action->extern_library, 32);
	VLOG_WARN("kahina. odp_execute_action execute_odp_vnf_action %d",0);
	if(open_library(name_library))
	{
	  error = call_function(action,packet);
	  VLOG_WARN("kahina. odp_execute_action call function done   %s",0);
	  // packet_set_ipv4(packet, 16843009, 16843009 , 4, 15);
	  close_library();
	}
}
//kahina
...
requires_datapath_assistance(const struct nlattr *a)
{
    ...
    case OVS_ACTION_ATTR_VNF: //Kahina
        return false;
                    ...
                    odp_execute_cb dp_execute_action)
{
    ...
    NL_ATTR_FOR_EACH_UNSAFE (a, left, actions, actions_len) {
        ...
        case OVS_ACTION_ATTR_OUTPUT:
        	VLOG_WARN("kahina odp_execute_action OVS_ACTION_ATTR_OUTPUT %d",0);
        case OVS_ACTION_ATTR_TUNNEL_PUSH:
        case OVS_ACTION_ATTR_TUNNEL_POP:
        case OVS_ACTION_ATTR_USERSPACE:
        case OVS_ACTION_ATTR_RECIRC:
        case OVS_ACTION_ATTR_CT:
        //kahina
		case OVS_ACTION_ATTR_VNF:
			VLOG_WARN("Amrane going through odp_execute_actions OVS_ACTION_ATTR_VNF");
			for (i = 0; i < cnt; i++) {
				char *str_pack;
				VLOG_WARN("kahina odp_execute OVS_ACTION_ATTR_VNF  %d",0);

				//struct ip_header ip_h=
				//packet_set_ipv4(packet, 0, 0, 8, 15);

				const struct ovs_action_vnf * action = nl_attr_get(a);  /* deplacer dans library externe*/

				//str_pack = ofpbuf_to_string(packet,  packet->size_);
				//VLOG_WARN(" kahina odp_execute OVS_ACTION_ATTR_VNF : before call packet_set_ipv4 %s", str_pack);
				//packet_set_ipv4(packet, 16843009, 16843009 , 4, 15);   //test

			   execute_odp_vnf_action(packets[i], action);
			   // packet_set_ipv4(packet, 16843009, 16843009 , 4, 15);
			  str_pack = dp_packet_to_string(packets[i],  packets[i]->size_);
			  VLOG_WARN(" kahina odp_execute OVS_ACTION_ATTR_VNF : after  call vnf action  %s", str_pack);

				//struct ovs_action_push_vlan *vlan;
				//ovs_vlan->vlan_tci=0x11111111;
