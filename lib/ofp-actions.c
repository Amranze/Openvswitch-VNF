...
/* Actions similar to standard actions are listed with the standard actions. */
	...
	/* NX1.0+(32): struct nx_action_vnf. */
	NXAST_RAW_VNF_ACTION,

    /* NX1.0+(36): struct nx_action_output_reg2. */
...
/* Action structure for NXAST_VNF_ACTION. */
struct nx_action_vnf
{
	ovs_be16 type;                  /* OFPAT_VENDOR. */
	ovs_be16 len;                   /* Length is 24. */
	ovs_be32 vendor;                /* NX_VENDOR_ID. */
	ovs_be16 subtype;               /* VNF_ACTION. */
  	uint8_t  pad[2];				/* I added 2 Pad because it was reading the bytes before */
	ovs_be32 vnf_id;               /* faire attention a  allignement sur 32 bits et 64 bits */
};
OFP_ASSERT(sizeof(struct nx_action_vnf) == 16);
//FIXME TRY TO KNOW WHAT IS SHE DOING WITH IT

static enum ofperr
decode_NXAST_RAW_VNF_ACTION(const struct nx_action_vnf *vnf,
		enum ofp_version ofp_version OVS_UNUSED, struct ofpbuf *out)
{
	VLOG_WARN("Kahina decode_  %"PRIu16" l %"PRIu32" ", htons(0), htonl(0));
	VLOG_WARN("Kahina decode_NXAST_RAW_VNF_ACTION len %d subtype %d type %d vendor %d vnf_id %d",
			ntohs(vnf->len), ntohs(vnf->subtype),
			ntohs(vnf->type), ntohs(vnf->vendor), ntohs(vnf->vnf_id));

	struct ofpact_vnf_action *vnf_action;
	vnf_action = ofpact_put_VNF_ACTION(out);
	if(ntohs(vnf->vnf_id) == 0){
		vnf_action->vnf_id = ntohl(vnf->vnf_id);
	} else {
		vnf_action->vnf_id = ntohs(vnf->vnf_id);
	}

	VLOG_WARN("Kahina >>>>> htonl %d vnf->vnf_id %d htons %d", vnf_action->vnf_id, vnf->vnf_id, htons(vnf->vnf_id));

	VLOG_WARN("Kahina >>>>>>>decode_NXAST_RAW_VNF_ACTION %d", vnf_action->vnf_id);

	//ofputil_put_VNF_ACTION(out)
	/*
	struct nx_action_vnf *vnf_action;
	vnf_action = ofpact_put_VNF_ACTION(out);
	VLOG_WARN("Kahina decode_NXAST_RAW_VNF_ACTION "
			"vnf_action %d nthol %d ntohs %d", vnf_action->vnf_id,
			ntohl(vnf_action->vnf_id), ntohs(vnf_action->vnf_id));

	vnf_action->vnf_id = ntohl(vnf->vnf_id);
	VLOG_WARN("Kahina decode_NXAST_RAW_VNF_ACTION "
			"vnf %d nthol %d ntohs %d", vnf->vnf_id,
			ntohl(vnf->vnf_id), ntohs(vnf->vnf_id));

	VLOG_WARN("Kahina >>>>>>>decode_NXAST_RAW_VNF_ACTION %d", vnf_action->vnf_id);
*/
	/*struct ofpact_vnf_action *vnf_action1;
	vnf_action1=ofpact_put_VNF_ACTION(out);
	VLOG_WARN("Amrane : vnf_from_openflow after mutex vnf_action1->vnf_id %d", vnf_action1->vnf_id);
	 */
/*
	struct ofpact_vnf_action *action_nf;
	action_nf = ofpact_put_VNF_ACTION(out);
	action_nf->vnf_id = ntohs(vnf_action->vnf_id);*/
	//FIXME (vnf_action->vnf_id) >= ENTRY_NBR
   if ((vnf_action->vnf_id) >= 100   )
	   return  OFPERR_OFPBAC_BAD_ARGUMENT;  // voir si possible envoyer erreur ds  experim msg
  /*
    vnf_action = put_VNF_ACTION(out);
	   if(vac->vnf_id >= ENTRY_NBR) {
		   vnf_action->vnf_id = ntohl(vac->vnf_id);
	   } else {
		   vnf_action->vnf_id = vac->vnf_id;
	   }
	   VLOG_WARN("Amrane : vnf_from_openflow after mutex vnf_action1->vnf_id %d", vnf_action->vnf_id);
	   if ((vnf_action->vnf_id) >= 100   )
		   //FIXME (vnf_action->vnf_id) >= ENTRY_NBR
	       return  OFPERR_OFPBAC_BAD_ARGUMENT;  // voir si possible envoyer erreur ds  experim msg
*/
...
static void
encode_VNF_ACTION(const struct ofpact_vnf_action *vnf_action,
                enum ofp_version ofp_version OVS_UNUSED, struct ofpbuf *out)
{
	/*put_OFPAT_VNF_ACTION(out, ofp_version, vnf_action->vnf_id);*/
	struct nx_action_vnf *action_nf = put_NXAST_VNF_ACTION(out);
	action_nf->vnf_id = htons(vnf_action->vnf_id);
	VLOG_WARN("Kahina encode_VNF_ACTION %d htons %d", vnf_action->vnf_id, action_nf->vnf_id);
	/*
	struct nx_action_vnf *vnf_action;
	vnf_action = ofputil_put_NXAST_VNF_ACTION(out);
	vnf_action->vnf_id=vnf->vnf_id;
	 */
}


static char * OVS_WARN_UNUSED_RESULT
parse_VNF_ACTION(char *arg, struct ofpbuf *ofpacts,
               enum ofputil_protocol *usable_protocols OVS_UNUSED)
{
	VLOG_WARN("Amrane ofpact_put_VNF_ACTION(ofpacts)->vnf_id %d",
			ofpact_put_VNF_ACTION(ofpacts)->vnf_id);
	VLOG_WARN("Amrane str_to_u32 %d",
	str_to_u32(arg, &ofpact_put_VNF_ACTION(ofpacts)->vnf_id));
	return str_to_u32(arg, &ofpact_put_VNF_ACTION(ofpacts)->vnf_id);
}

static void
format_VNF_ACTION(const struct ofpact_vnf_action *a, struct ds *s)
{
	 ds_put_format(s, "vnf_action:%"PRIu32, a->vnf_id);
}
                       ...
                       struct ofpbuf *ofpacts)
{
    ...
    learn->match_nf = ntohs(nal->match_nf);
    VLOG_WARN("Amrane decode_NXAST_RAW_LEARN match_nf %d or without ntohs %d "
    		" ", learn->match_nf, nal->match_nf);
             ...
             enum ofp_version ofp_version OVS_UNUSED, struct ofpbuf *out)
{
    ...
    nal->match_nf = htons(learn->match_nf);
    VLOG_WARN("Amrane encode_LEARN match_nf %d or without ntohs %d",
    		nal->match_nf, learn->match_nf);
