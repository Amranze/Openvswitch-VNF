...
odp_action_len(uint16_t type)
{
    ...
    case OVS_ACTION_ATTR_VNF: return sizeof(struct ovs_action_vnf); //KAHINA
...
{
    {
        ...
        if (ovs_scan(s, "%"SCNi32"%n", &port, &n)) {
        	VLOG_WARN("kahina...nl_msg_put_u32 output ovs_scan  %d ",1);
    ...
    if (port_names) {
        ...
        if (node) {
        	VLOG_WARN("kahina...nl_msg_put_u32 output if node  %d ",1);
