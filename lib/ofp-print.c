...
ofp_print_flow_mod(struct ds *s, const struct ofp_header *oh, int verbosity)
{
    ...
    if (fm.match_nf != 0) {
        ds_put_format(s, "match_nf:%"PRIu16" ", fm.match_nf);
    }
...
ofp_print_flow_removed(struct ds *string, const struct ofp_header *oh)
{
    ...
    ds_put_format(string, " reason=%s",
                  ofp_flow_removed_reason_to_string(fr.reason, reasonbuf,
                                                    sizeof reasonbuf));
    if (fr.match_nf != 0) {
        ds_put_format(string, " match_nf=%"PRIu16", ", fr.match_nf);
    }
    if (fr.table_id != 255) {
...
ofp_print_flow_stats(struct ds *string, struct ofputil_flow_stats *fs)
{
    ...
    if (fs->match_nf != 0) {
        ds_put_format(string, "match_nf=%"PRIu16", ", fs->match_nf);
    }
                                  ...
                                  const struct ofp_header *oh)
{
    ...
    for (;;) {
        ...
        if (update.match_nf != 0) {
            ds_put_format(string, " match_nf=%"PRIu16,
                          update.match_nf);
        }
