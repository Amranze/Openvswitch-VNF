                        ...
                        ofp_port_t max_port, uint8_t max_table)
{
    ...
    if (raw == OFPRAW_OFPT11_FLOW_MOD) {
        ...
        fm->match_nf = ntohs(ofm->match_nf);
        VLOG_WARN("Amrane :1 fm->match_nf :  :  %d  without ntohs %d ",
                		fm->match_nf, ofm->match_nf);
        ...
        if (raw == OFPRAW_OFPT10_FLOW_MOD) {
            ...
            /* Translate the message. */
            fm->match_nf = ofm->match_nf;
            VLOG_WARN("Amrane :2 fm->match_nf :  %d",fm->match_nf);
            ...
            fm->match_nf = ntohs(nfm->match_nf);
            VLOG_WARN("Amrane :3 fm->match_nf: %d !! %d",fm->match_nf, nfm->match_nf);
                        ...
                        enum ofputil_protocol protocol)
{
    ...
    case OFPUTIL_P_OF15_OXM: {
        ...
        ofm->match_nf = htons(fm->match_nf);
        //ofm->match_nf = fm->match_nf; //XXX i changed this one
        VLOG_WARN("Amrane ofp-uttil OF15 %d fm %d", ofm->match_nf, fm->match_nf);
    ...
    case OFPUTIL_P_OF10_STD_TID: {
        ...
        ofm->hard_timeout = htons(fm->hard_timeout);
        ofm->priority = htons(fm->priority);
        ofm->match_nf = fm->match_nf;
        /* it was ofm->match_nf = htons(fm->match_nf); but i had a problem
         * of inversion of byte so i removed htons */
        // FIXME remove htons
        VLOG_WARN("Amrane ofp-uttil OF10 ofm %d fm %d", ofm->match_nf, fm->match_nf);
        ofm->buffer_id = htonl(fm->buffer_id);
    ...
    case OFPUTIL_P_OF10_NXM_TID: {
        ...
        nfm->match_nf = htons(fm->match_nf);
        VLOG_WARN("Amrane ofp-uttil NX %d fm %d", nfm->match_nf, fm->match_nf);
    ...
    ofpmsg_update_length(msg);
    char *str_pack = ofpbuf_to_string(msg,  msg->size);
    VLOG_WARN("Amrane : msg buffer  :  %s ",str_pack);
    return msg;
}
                                ...
                                struct ofpbuf *ofpacts)
{
    ...
    if (!msg->size) {
        ...
        fs->match_nf = ntohs(ofs->match_nf);
        VLOG_WARN("Amrane ofputil_decode_flow_stats_reply ofp11"
        		"fs %d ofs %d", fs->match_nf, ofs->match_nf);
        ...
        fs->priority = ntohs(ofs->priority);
        fs->match_nf = ntohs(ofs->match_nf);
        VLOG_WARN("Amrane ofputil_decode_flow_stats_reply ofp10 "
        		"fs %d ofs %d", fs->match_nf, ofs->match_nf);
        fs->table_id = ofs->table_id;
        fs->duration_sec = ntohl(ofs->duration_sec);
        fs->duration_nsec = ntohl(ofs->duration_nsec);
        ...
        fs->match_nf = ntohs(nfs->match_nf);
        VLOG_WARN("Amrane ofputil_decode_flow_stats_reply nxst "
        		"fs %d nfs %d", fs->match_nf, nfs->match_nf);
                                ...
                                struct ovs_list *replies)
{
    ...
    if (raw == OFPRAW_OFPST11_FLOW_REPLY || raw == OFPRAW_OFPST13_FLOW_REPLY) {
        ...
        ofs->match_nf = htons(fs->match_nf);
        VLOG_WARN("Amrane ofputil_append_flow_stats_reply OFP11 "
        		"ofs %d fs %d", ofs->match_nf, fs->match_nf);
        ...
        ofs->priority = htons(fs->priority);
        ofs->match_nf = htons(fs->match_nf);
        VLOG_WARN("Amrane ofputil_append_flow_stats_reply OFP10 "
        		"ofs %d fs %d", ofs->match_nf, fs->match_nf);
        ofs->idle_timeout = htons(fs->idle_timeout);
        ofs->hard_timeout = htons(fs->hard_timeout);
        memset(ofs->pad2, 0, sizeof ofs->pad2);
        ...
        nfs->match_nf = htons(fs->match_nf);
        VLOG_WARN("Amrane ofputil_append_flow_stats_reply nx "
        		"ofs %d fs %d", nfs->match_nf, fs->match_nf);
                            ...
                            const struct ofp_header *oh)
{
    ...
    if (raw == OFPRAW_OFPT11_FLOW_REMOVED) {
        ...
        fr->match_nf = ntohs(ofr->match_nf);
        VLOG_WARN("Amrane ofputil_decode_flow_removed OFP12 "
        		"fr %d ofr %d", fr->match_nf, ofr->match_nf);
        ...
        fr->priority = ntohs(ofr->priority);
        fr->match_nf = ntohs(ofr->match_nf);
        VLOG_WARN("Amrane ofputil_decode_flow_removed OFP10 "
        		"fr %d ofr %d", fr->match_nf, ofr->match_nf);
        fr->cookie = ofr->cookie;
        fr->reason = ofr->reason;
        fr->table_id = 255;
        ...
        fr->match_nf = ntohs(nfr->match_nf);
        VLOG_WARN("Amrane ofputil_decode_flow_removed NX "
        		"fr %d nfr %d", fr->match_nf, nfr->match_nf);
                            ...
                            enum ofputil_protocol protocol)
{
    ...
    case OFPUTIL_P_OF15_OXM: {
        ...
        ofr->match_nf = htons(fr->match_nf);
        VLOG_WARN("Amrane ofputil_encode_flow_removed OFP12 "
        		"ofr %d fr %d", ofr->match_nf, fr->match_nf);
        ofr->reason = reason;
    ...
    case OFPUTIL_P_OF10_STD_TID: {
        ...
        ofr->priority = htons(fr->priority);
        ofr->match_nf = htons(fr->match_nf);
        VLOG_WARN("Amrane ofputil_encode_flow_removed OFP10 "
        		"ofr %d fr %d", ofr->match_nf, fr->match_nf);
        ofr->reason = reason;
        ofr->duration_sec = htonl(fr->duration_sec);
        ofr->duration_nsec = htonl(fr->duration_nsec);
    ...
    case OFPUTIL_P_OF10_NXM_TID: {
        ...
        nfr->match_nf = htons(fr->match_nf);
        VLOG_WARN("Amrane ofputil_encode_flow_removed NX "
        		"nfr %d fr %d", nfr->match_nf, fr->match_nf);
        nfr->reason = reason;
                           ...
                           struct ofpbuf *msg, struct ofpbuf *ofpacts)
{
    ...
    if (update->event == NXFME_ABBREV) {
        ...
        update->match_nf = ntohs(nfuf->match_nf);
        VLOG_WARN("Amrane ofputil_decode_flow_update NX "
        		"update %d nfuf %d", update->match_nf, nfuf->match_nf);
                           ...
                           struct ovs_list *replies)
{
    ...
    if (update->event == NXFME_ABBREV) {
        ...
        nfuf->match_nf = htons(update->match_nf);
        VLOG_WARN("Amrane ofputil_append_flow_update NX "
        		"nfuf %d update %d", nfuf->match_nf, update->match_nf);
