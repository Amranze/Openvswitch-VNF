                ...
                const struct cls_conjunction conj[], size_t n)
{
    ...
    *CONST_CAST(int *, &cls_match->match_nf) = rule->match_nf;
...
static const struct cls_match *find_match_wc(const struct cls_subtable *,
											 ...
											 const struct vnftable *,
											 const struct dp_packet *);

static const struct cls_match *find_match_wc_revalidate(const struct cls_subtable *,
                                             cls_version_t version,
                                             const struct flow *,
                                             struct trie_ctx *,
                                             unsigned int n_tries,
...
static inline void
cls_rule_init__(struct cls_rule *rule, unsigned int priority, unsigned int match_nf)
{
    rculist_init(&rule->node);
    *CONST_CAST(int *, &rule->priority) = priority;
    *CONST_CAST(int *, &rule->match_nf) = match_nf;
    rule->cls_match = NULL;
}
...
void
cls_rule_init(struct cls_rule *rule, const struct match *match,
		int priority, int match_nf)
{
    cls_rule_init__(rule, priority, match_nf);
...
cls_rule_init_from_minimatch(struct cls_rule *rule,
                             const struct minimatch *match, int priority, int match_nf)
{
    cls_rule_init__(rule, priority, match_nf);
...
{
    cls_rule_init__(dst, src->priority, src->match_nf);
...
{
    return a->priority == b->priority && a->match_nf == b->match_nf && minimatch_equal(&a->match, &b->match);
}
...
//FIXME Amrane i need to chek if i will add match_nf
/* Returns true if 'rule' matches every packet, false otherwise. */
bool
cls_rule_is_catchall(const struct cls_rule *rule)
{
	//VLOG_WARN("Amrane cls_rule_is_catchall match_nf %d ",rule->match_nf);
	if(rule->match_nf != 0) {
		return false;
	}
...
classifier_lookup__(const struct classifier *cls,
			 	  	cls_version_t version, struct flow *flow, struct flow_wildcards *wc,
                    bool allow_conjunctive_matches,const struct dp_packet *packet,
					const struct vnftable *vnf_table)
{
    ...
    PVECTOR_FOR_EACH_PRIORITY (subtable, hard_pri, 2, sizeof *subtable,
        ...
        match = find_match_wc(subtable, version, flow, trie_ctx, cls->n_tries,
                              wc, vnf_table, packet);
        if(match)
        	VLOG_WARN("Amrane PVECTOR_FOR_EACH_PRIORITY prio %d m %d", match->priority, match->match_nf);
        if (!match || match->priority <= hard_pri) {
        	VLOG_WARN("Amrane if before continue");
            continue;
        }
        VLOG_WARN("Amrane if after continue");
        ...
        if (!conj_set) {
            ...
            VLOG_WARN("Amrane else allow_conjunctive_matches");
        }
    }
    ...
    if (hard_pri >= soft_pri) {
    	...
    	VLOG_WARN("Amrane here hard ? hard->cls_rule : NULL");
    ...
    for (;;) {
        ...
        for (int i = 0; i < n_soft; i++) {
            ...
            if (soft[i]->priority == soft_pri
                ...
                flow->conj_id = id;
                rule = classifier_lookup__(cls, version, flow, wc, false, packet, vnf_table);
    ...
    VLOG_WARN("Amrane the last hard ? hard->cls_rule : NULL");
    return hard ? hard->cls_rule : NULL;
}
 ...
 * Any changes are restored before returning. */
static const struct cls_rule *
classifier_lookup_revalidate(const struct classifier *cls,
			 	  	cls_version_t version, struct flow *flow, struct flow_wildcards *wc,
                    bool allow_conjunctive_matches)
{
    ...
    PVECTOR_FOR_EACH_PRIORITY (subtable, hard_pri, 2, sizeof *subtable,
        ...
        /* Skip subtables with no match, or where the match is lower-priority
         * than some certain match we've already found. */
        match = find_match_wc_revalidate(subtable, version, flow, trie_ctx, cls->n_tries,
                              wc);
        if(match)
        	VLOG_WARN("Amrane PVECTOR_FOR_EACH_PRIORITY prio %d m %d", match->priority, match->match_nf);
        if (!match || match->priority <= hard_pri) {
    ...
    for (;;) {
        ...
        for (int i = 0; i < n_soft; i++) {
            ...
            if (soft[i]->priority == soft_pri
                ...
                flow->conj_id = id;
                rule = classifier_lookup_revalidate(cls, version, flow, wc, false);
                flow->conj_id = saved_conj_id;
...
classifier_lookup(const struct classifier *cls,
				  cls_version_t version, struct flow *flow, struct flow_wildcards *wc,
				  const struct dp_packet *packet,const struct vnftable *vnf_table)
{
	/* Amrane I added struct ofproto_dpif *ofproto && const struct dp_packet *packet*/
	if(packet)
		return classifier_lookup__(cls, version, flow, wc, true, packet, vnf_table);
	else
		return classifier_lookup_revalidate(cls, version, flow, wc, true);
}
                             ...
                             cls_version_t version)
{
    ...
    if (!subtable) {
    	//VLOG_WARN("Amrane classifier_find_rule_exactly !subtable");
        return NULL;
    }
    ...
    head = find_equal(subtable, target->match.flow,
                      miniflow_hash_in_minimask(target->match.flow,
                                                target->match.mask, 0));
    if (!head) {
    	//VLOG_WARN("Amrane classifier_find_rule_exactly !head");
        return NULL;
    }
	//VLOG_WARN("Amrane collect_rules_strict prio %d match_nf %d", head->priority, head->match_nf);
                              ...
                              cls_version_t version)
{
    ...
    cls_rule_init(&cr, target, priority, 0); //Amrane i added 0 for match_nf
...
{
	if(target){
		if(target->match_nf && target->match_nf != 0){
			VLOG_WARN(" Amrane rule_matches rule priority %d "
					"match_nf %d and target match_nf %d ", rule->priority, rule->match_nf, target->match_nf);
			if(rule->match_nf == target->match_nf){
				VLOG_WARN(" Amrane rule_matches true equal");
				return true;
			} else {
				return false;
			}
...
find_match(const struct cls_subtable *subtable, cls_version_t version,
           const struct flow *flow, uint32_t hash, const struct vnftable *vnf_table,
        ...
        if (OVS_LIKELY(miniflow_and_mask_matches_flow(&head->flow,
            ...
            CLS_MATCH_FOR_EACH (rule, head) {
                if (OVS_LIKELY(cls_match_visible_in_version(rule, version))) {
                    if (OVS_LIKELY(cls_match_visible_in_version(rule, version))) {
                    	VLOG_WARN("Amrane cls_match_visible_in_version rule prio %d", rule->priority);
                    	if(rule->match_nf != 0){
    						struct ovs_action_vnf *action = malloc(sizeof(*action));
    						strncpy(action->extern_library ,vnf_table->vnf_table[rule->match_nf].extern_library,32);
    						strncpy(action->fnct_name,vnf_table->vnf_table[rule->match_nf].fnct_name,32);
    						action->fnct_parametre = vnf_table->vnf_table[rule->match_nf].internal_parameters;
    						action->external_parameters = vnf_table->vnf_table[rule->match_nf].external_parameters;
    						int match_nf = match_nf_function(packet, action);
    						free(action);
    						switch(match_nf){
    							case 0:
    								VLOG_WARN("Amrane there is a no match");
    								break;
    							case 1:
    								VLOG_WARN("Amrane there is a match");
    								return rule;
    							case 2:
    								VLOG_WARN("Amrane there is a problem");
    								//return NULL;
    								break;
    						}
                    	}
                    	else return rule;
                    	VLOG_WARN("Amrane still looking we are in rule prio %d", rule->priority);
                    }
                }
            }
        }
...
static inline const struct cls_match *
find_match_revalidate(const struct cls_subtable *subtable, cls_version_t version,
...
find_match_wc(const struct cls_subtable *subtable,
			  cls_version_t version, const struct flow *flow,
			  struct trie_ctx trie_ctx[CLS_MAX_TRIES], unsigned int n_tries,
			  struct flow_wildcards *wc,const struct vnftable *vnf_table,
			  const struct dp_packet *packet)
{
    if (OVS_UNLIKELY(!wc)) {
    	VLOG_WARN("Amrane here OVS_UNLIKELY");
        return find_match(subtable, version, flow,
                          flow_hash_in_minimask(flow, &subtable->mask, 0), vnf_table, packet);
    }
    ...
    for (i = 0; i < subtable->n_indices; i++) {
    	VLOG_WARN("Amrane finish early by checking fields in segments.");
        ...
        if (!cmap_node_next(inode)) {
            ...
            VLOG_WARN("Amrane ASSIGN_CONTAINER match_nf %d prio %d",
                            			head->match_nf, head->priority);
            if (miniflow_and_mask_matches_flow_wc(&head->flow, &subtable->mask,
                                                  flow, wc)) {
                /* Return highest priority rule that is visible. */
                CLS_MATCH_FOR_EACH (rule, head) {
                	VLOG_WARN("Amrane CLS_MATCH_FOR_EACH match_nf %d prio %d",
                	                			rule->match_nf, rule->priority);
                	VLOG_WARN("Amrane it goes from here CLS_MATCH_FOR_EACH");

                	if(rule && rule->match_nf != 0){
                		struct ovs_action_vnf *action = malloc(sizeof(*action));
						strncpy(action->extern_library ,vnf_table->vnf_table[rule->match_nf].extern_library,32);
						strncpy(action->fnct_name,vnf_table->vnf_table[rule->match_nf].fnct_name,32);
						action->fnct_parametre = vnf_table->vnf_table[rule->match_nf].internal_parameters;
    ...
    if (check_tries(trie_ctx, n_tries, subtable->trie_plen,
                    subtable->index_maps[i], flow, wc)) {
    	VLOG_WARN("Amrane no_match 3");
        goto no_match;
    }
    hash = flow_hash_in_minimask_range(flow, &subtable->mask,
                                       subtable->index_maps[i],
                                       &mask_offset, &basis);
    rule = find_match(subtable, version, flow, hash, vnf_table, packet);

    /*if(rule){
    	VLOG_WARN("Amrane it goes from here rule && rule match_nf");
		int match_nf = check_next_rule(rule, version, vnf_table, packet);
		switch(match_nf){
			case 0:
				VLOG_WARN("Amrane there is a no match");
				rule = check_next_rule(rule, version, vnf_table, packet);
				if(rule) {
					flow_wildcards_fold_minimask(wc, &subtable->mask);
					VLOG_WARN("Amrane there is a rule ");
					return rule;
				}
				goto no_match;
			case 1:
    ...
    /*if(rule && rule->match_nf != 0){
    	VLOG_WARN("Amrane rule an,ddlfkdl ");
		struct ovs_action_vnf *action = malloc(sizeof(*action));
		strncpy(action->extern_library ,vnf_table->vnf_table[rule->match_nf].extern_library,32);
		strncpy(action->fnct_name,vnf_table->vnf_table[rule->match_nf].fnct_name,32);
		action->fnct_parametre = vnf_table->vnf_table[rule->match_nf].internal_parameters;
		action->external_parameters = vnf_table->vnf_table[rule->match_nf].external_parameters;
		int match_nf = execute_match_nf(packet, action);
		VLOG_WARN("Amrane match_nf %d", match_nf);
		free(action);
		//free(vnf_table);
		switch(match_nf){
			case 0:
    ...
    if (!rule && subtable->ports_mask_len) {
    	 VLOG_WARN("Amrane !rule && subtable->ports_mask_len");
...
static const struct cls_match *
find_match_wc_revalidate(const struct cls_subtable *subtable, cls_version_t version,
              const struct flow *flow, struct trie_ctx trie_ctx[CLS_MAX_TRIES],
              unsigned int n_tries, struct flow_wildcards *wc)
{
    if (OVS_UNLIKELY(!wc)) {
        return find_match_revalidate(subtable, version, flow,
    ...
    hash = flow_hash_in_minimask_range(flow, &subtable->mask,
                                       subtable->index_maps[i],
                                       &mask_offset, &basis);
    rule = find_match_revalidate(subtable, version, flow, hash);
    if (!rule && subtable->ports_mask_len) {
        /* The final stage had ports, but there was no match.  Instead of
         * unwildcarding all the ports bits, use the ports trie to figure out a
         * smaller set of bits to unwildcard. */
...
char *remove_begin_space_match_nf(char *str, int len) {
	char *s = str;
	int index=0, i=0;
	VLOG_WARN("Name of the string : %s  ",str );
	while(*s==0x20){
		s++;
		index++;
	}
	char *fn_name= malloc ((len-index+1)*sizeof(char));
...
int  open_library_match_nf(char *library_Name){
	char *extern_library= "/home/air/lib_vnffunctions/";
	char *library = malloc(strlen(extern_library)+strlen(library_Name)+1);
	strcpy(library, extern_library);
	strcat(library, library_Name);
	vnf_lib=dlopen(library,RTLD_LAZY);

	if(!vnf_lib) {
		VLOG_WARN("Amrane dlopen error %s \n",dlerror());
		return false;
	}
	return true;
...
void close_library_match_nf()
{
	dlclose(vnf_lib);
}

int call_function_match_nf(struct ovs_action_vnf *action, struct dp_packet  *p)
{
	char * error_;
	int x=0;char *name;
	int (*func_call)(struct dp_packet * packet, struct ofpbuf *parametres);
	//dlerror();
	name=remove_begin_space_match_nf(action->fnct_name, 32);
	func_call=dlsym(vnf_lib,name);
	if(!func_call) {
		VLOG_WARN("Amrane dlsym error %s", dlerror());
		return 1;
	}
	x=(*func_call)(p, action->fnct_parametre );
	VLOG_WARN("The result of the calling is %d ",x);
	return x;
}


int match_nf_function(struct dp_packet *packet, struct ovs_action_vnf *action)
{
	int error;
	char *name_library;
	name_library = remove_begin_space_match_nf(action->extern_library, 32);
	if(open_library_match_nf(name_library))
	{
	  error = call_function_match_nf(action,packet);
	  close_library_match_nf();
	  return error;
	}
	return 2;
}
