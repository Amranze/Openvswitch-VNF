              ...
              struct ofputil_flow_mod *fm, struct ofpbuf *ofpacts)
{
    ...
    fm->match_nf = learn->match_nf;
    VLOG_WARN("Amrane learn_execute match_nf %d", learn->match_nf);
    fm->cookie = htonll(0);
...
learn_parse__(char *orig, char *arg, struct ofpbuf *ofpacts)
{
    ...
    while (ofputil_parse_key_value(&arg, &name, &value)) {
        ...
        } else if (!strcmp(name, "match_nf")) {
            learn->match_nf = atoi(value);
            VLOG_WARN("Amrane learn_parse__ atoi match_nf %d", learn->match_nf);
...
learn_format(const struct ofpact_learn *learn, struct ds *s)
{
    ...
    if (learn->match_nf != 0) {
       ds_put_format(s, ",match_nf=%"PRIu16, learn->match_nf);
    }
    if (learn->flags & NX_LEARN_F_SEND_FLOW_REM) {
