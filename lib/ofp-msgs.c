...
//kahina 4 remplace par 70 and in build-aux/extract-ofp-msgs
#define OFPT_VENDOR 70
//kahina
               ...
               const struct ofp_header *oh, size_t length)
{
    ...
    hdrs->type = oh->type;
    // kahina
	if (hdrs->type == OFPTYPE_EXPERIMENTER) {
		// Get vendor.
		const struct ofp12_experimenter_stats_header *exph;

		if (length < sizeof *exph) {
			return OFPERR_OFPBRC_BAD_LEN;
		}

		exph = (const struct ofp12_experimenter_stats_header *) oh;
...
ofphdrs_len(const struct ofphdrs *hdrs)
{
	...
	if (hdrs->type == OFPTYPE_EXPERIMENTER) {
	        //return
		size= sizeof(struct ofp_header)+sizeof(struct ofp12_experimenter_stats_header);
		return size;
	}
   //kahina end
